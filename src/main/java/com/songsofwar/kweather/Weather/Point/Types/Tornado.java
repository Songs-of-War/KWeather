package com.songsofwar.kweather.Weather.Point.Types;
...
import com.songsofwar.kweather.Commands;
import com.songsofwar.kweather.LanguageLoader;
import com.songsofwar.kweather.Weather.WeatherHandler;
import com.songsofwar.kweather.Weather.Point.PointWeather;
import com.songsofwar.kweather.Weather.Point.Types.Methods.TornadoDirection;
...
public class Tornado extends PointWeather
{
	...
	public static void loadConfig(FileConfiguration config)
	{
		...
		name = KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado");
	...
	private static void setUpBreakableArray(List<String> materialnames)
	{
		for(String matname : materialnames) {
			try {
				...
				Bukkit.getServer().getLogger().warning(KWeather.getPrefix() + "There is no material name by " + matname);
			}
		}
	}
	...
	public void startTornado(int dur)
	{
		...
		}.runTaskTimer(KWeather.getPlugin(), 0, 2);
	}
	...
	{
		if(args[1].equalsIgnoreCase(KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado"))
				|| args[1].equalsIgnoreCase(KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.FireTornado"))) {
			if(args.length == 2) {
				if(sender instanceof Player) {
					...
					if(args[1].equalsIgnoreCase(KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.FireTornado"))) {
						t.setFire(true);
					}
				} else {
					if(!Bukkit.getOnlinePlayers().isEmpty()) {
						...
						String msg = KWeather.getLanguageLoader().lineInterpreter(
								KWeather.getLanguageLoader().getLanguage().getString("ChatMessages.noonlinenotornado"),
								"");
				...
				try {
					...
					String msg = KWeather.getLanguageLoader().lineInterpreter(KWeather.getLanguageLoader().getLanguage().getString("ChatMessages.needint"), "");
					Commands.sendMessage(msg, sender);
				}
			}
			return true;
	...
	public static boolean onCommandStop(CommandSender sender, String[] args) 
	{
		...
		if(args[1].equalsIgnoreCase(KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado"))
				|| args[1].equalsIgnoreCase(KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.FireTornado"))) {
			String msg = KWeather.getLanguageLoader().lineInterpreter(
					KWeather.getLanguageLoader().getLanguage().getString("ChatMessages.stopweather"),
					KWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado"));
			Commands.sendMessage(msg, sender);
		...
		if(args.length == 2) {
			if(sender instanceof Player) {
				...
				if(!Bukkit.getOnlinePlayers().isEmpty()) {
					...
					String msg = KWeather.getLanguageLoader().lineInterpreter(language.getString("ChatMessages.noonlinenotornado"),
							"");
			...
			try {
				...
				String msg = KWeather.getLanguageLoader().lineInterpreter(language.getString("ChatMessages.needint"), "");
				sendMessage(msg, sender);
			}
		}
		return true;
