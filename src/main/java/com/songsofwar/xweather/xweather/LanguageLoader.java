...
public class LanguageLoader 
{
	...
	{
		File languagefile = new File(XWeather.getPlugin().getDataFolder(), "language.yml");
		if(!languagefile.exists()) {
			Bukkit.getServer().getLogger().info(XWeather.getPrefix() + "Could not find language.yml, making a new one now");
			XWeather.getPlugin().saveResource("language.yml", false);
		}
	...
	{
		Bukkit.getServer().getLogger().info(XWeather.getPrefix() + "You are missing some options in your language.yml.");
		Bukkit.getServer().getLogger().info(XWeather.getPrefix() + "Saving old language file and making a new one.");
		File oldl = new File(XWeather.getPlugin().getDataFolder(), "oldlanguage.yml");
		if(!oldl.exists()) {
			try {
				oldl.createNewFile();
			} catch (IOException e) {
		...
		XWeather.getPlugin().saveResource("language.yml", true);
		File languagefile = new File(XWeather.getPlugin().getDataFolder(), "language.yml");
		language = YamlConfiguration.loadConfiguration(languagefile);
	}
	...
	private boolean isMissing()
	{
		YamlConfiguration deflang = YamlConfiguration.loadConfiguration(
				new InputStreamReader(XWeather.getPlugin().getResource("language.yml")));
	...
	{
		String message = ChatColor.translateAlternateColorCodes('&', XWeather.getLanguageLoader().lineInterpreter(
				XWeather.getLanguageLoader().getLanguage().getString(langsection), special));
	...
	{
		String message = ChatColor.translateAlternateColorCodes('&', XWeather.getLanguageLoader().lineInterpreter(
				XWeather.getLanguageLoader().getLanguage().getString(langsection), special, varmatch));
		if(sendtoconsole) {Bukkit.getServer().getLogger().info(ChatColor.stripColor(message));}
		if(p != null && p.isOnline()) {
			p.sendMessage(message);
		}
