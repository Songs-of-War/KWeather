...
import com.songsofwar.xweather.xweather.Weather.World.Puddle;
...
public class Events implements Listener
{
	...
	public void onJoin(PlayerJoinEvent e)
	{
		...
		if(p.hasPermission("xweatherplus.admin") || p.isOp()) {
			...
			//Updater update = XWeather.getUpdater();
	...
	public void onPlayerDeath(PlayerDeathEvent e)
	{
		...
		if(hailstonedeaths.contains(p.getUniqueId())) {
			...
			e.setDeathMessage(XWeather.getLanguageLoader().lineInterpreter(
					XWeather.getLanguageLoader().getLanguage().getString("ChatMessages.hailstonedeath"),
					p.getName()));
		}
	}
