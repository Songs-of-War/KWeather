package com.songsofwar.xweather.xweather.Weather.Point.Types;
...
import com.songsofwar.xweather.xweather.Weather.Point.Types.Methods.TornadoDirection;
...
public class Tornado extends PointWeather
{
	...
	public static void loadConfig(FileConfiguration config)
	{
		...
		name = XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado");
	...
	private static void setUpBreakableArray(List<String> materialnames)
	{
		for(String matname : materialnames) {
			try {
				...
				Bukkit.getServer().getLogger().warning(XWeather.getPrefix() + "There is no material name by " + matname);
			}
		}
	}
	...
	public void startTornado(int dur)
	{
		...
		}.runTaskTimer(XWeather.getPlugin(), 0, 2);
	}
	...
	{
		if(args[1].equalsIgnoreCase(XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado"))
				|| args[1].equalsIgnoreCase(XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.FireTornado"))) {
			if(args.length == 2) {
				if(sender instanceof Player) {
					...
					if(args[1].equalsIgnoreCase(XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.FireTornado"))) {
						t.setFire(true);
					}
				} else {
					if(!Bukkit.getOnlinePlayers().isEmpty()) {
						...
						String msg = XWeather.getLanguageLoader().lineInterpreter(
								XWeather.getLanguageLoader().getLanguage().getString("ChatMessages.noonlinenotornado"),
								"");
				...
				try {
					...
					String msg = XWeather.getLanguageLoader().lineInterpreter(XWeather.getLanguageLoader().getLanguage().getString("ChatMessages.needint"), "");
					Commands.sendMessage(msg, sender);
				}
			}
			return true;
	...
	public static boolean onCommandStop(CommandSender sender, String[] args) 
	{
		...
		if(args[1].equalsIgnoreCase(XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado"))
				|| args[1].equalsIgnoreCase(XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.FireTornado"))) {
			String msg = XWeather.getLanguageLoader().lineInterpreter(
					XWeather.getLanguageLoader().getLanguage().getString("ChatMessages.stopweather"),
					XWeather.getLanguageLoader().getLanguage().getString("WeatherTypes.Tornado"));
			Commands.sendMessage(msg, sender);
		...
		if(args.length == 2) {
			if(sender instanceof Player) {
				...
				if(!Bukkit.getOnlinePlayers().isEmpty()) {
					...
					String msg = XWeather.getLanguageLoader().lineInterpreter(language.getString("ChatMessages.noonlinenotornado"),
							"");
			...
			try {
				...
				String msg = XWeather.getLanguageLoader().lineInterpreter(language.getString("ChatMessages.needint"), "");
				sendMessage(msg, sender);
			}
		}
		return true;
