package com.songsofwar.xweather.xweather.Weather;
...
public class Wind 
{
	...
	{
		if(alreadyrunning) {
			Bukkit.getServer().getLogger().info(XWeather.getPrefix() + "A server restart is required for some changes to occur.");
			return;
		}
		alreadyrunning = true;
		...
		}.runTaskTimer(XWeather.getPlugin(), updateperiod, updateperiod); //Might need to change to synchronously
	}
	...
	public static double[] getParticleVelocity(World world, double ymod) {
		...
		if(windobject == null) {
			Bukkit.getServer().getLogger().warning(XWeather.getPrefix() + "World does not have a wind instance assigned to it!");
			return null;
		}
